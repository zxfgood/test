window.onload=function(){
			...
			success : function(data) {
				...
				if(role=="管理员"){
					window.location.href="/compantchat/manger/mindex";
						...
						alert("权限不够");
						//window.location.href="/compantchat/repair/repairindex";
					}
	        }
		});
	}
	...
	/*办公维修的新增话题*/
		...
		//话题详情
		function btnPop_speakDetail(rid){
			//console.log(3333);
			//var mid=mid
		...
		function add() {
			...
			$.ajax({
				type:"POST",
				url:'/compantchat/repair/addrepair',
				async:true, 
				data:"repaircontent="+ textvalue,
				dataType: 'json',
				 success: function(data) {
	...
	//查看详情
	    function repairshow(rid) {
	    	//alert(mid);
		$.ajax({
			type:"GET",
			url:'/compantchat/repair/repairdetail',
			async:true, 
			data:"rid="+rid,
			dataType: 'json',
			 success: function(data) {
				 ...
				 var rreply=data.rreplay;
				 document.getElementById("TextValue1").innerHTML=rcontent ;
				 document.getElementById("rreply").value=rreply ;
				 document.getElementById("state").value=rstate ;
	...
	function btnPop_reply(rid){
	    ...
	    $("#pop_cov2").append("<div id='modlue2'>  " +
	        ...
	        "    </div>" +
	        "    <div class='MyBtn2'>" +
	        "        <button id='confirm2' onclick='return btnClose_onclick2("+rid+")' >确定</button>" +
	        "    </div>" +
	        "</div>"
	    );
	    show2(rid);  
		...
		$.ajax({
			...
			dataType: 'json',
			 success: function(data) {
				 ...
				 var rstate=data.rstate;
				 var rreply=data.rreplay;
				 document.getElementById("TextValue2").innerHTML=rcontent ;
		...
		function reply2(rid) {
			var rreply=$("#mreply").val();
		$.ajax({
			type:"POST",
			url:'/compantchat/repair/replyrepair',
			async:true, 
			data:"rid="+rid+"&rreply="+rreply,
			dataType: 'json',
			 success: function(data) {
				 window.location.href="/compantchat/repair/show";
			} 
			});
		}
	//关闭弹窗的函数
	function btnClose_onclick2(rid) {
		reply2(rid);
